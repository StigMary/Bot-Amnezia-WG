...
def register(bot: telebot.TeleBot):
    ...
    # ── /start — единая точка входа для всех ────────────────────────────────
    ...
    def cmd_start(message):
        ...
        if uid == cfg.admin_id:
            ...
            bot.reply_to(message, "👑 ЦУП v3.0", reply_markup=markup)
            return

        # ── Клиент: проверка группы ───────────────────────────────────────
        if cfg.group_chat_id != 0:
            try:
                ...
                logger.warning(f"Проверка группы /start uid={uid}: {e}")
                bot.reply_to(message, "⛔ Доступ запрещён.\nВы не состоите в закрытой группе.")
                return

        # ── Клиент: показ статуса ─────────────────────────────────────────
        from database import get_user_by_tg_id
        from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton as IBt
        ...
        if not user:
            bot.send_message(
                message.chat.id,
                "👋 Привет!\n\nВаш профиль ещё настраивается администратором.\n"
                "Как только всё будет готово — напишите /start снова."
            )
            return
        ...
        if paid_until:
            try:
                ...
                if days_left > 0:
                    status = f"✅ *Активен*\nОплачено до: `{exp.strftime('%d.%m.%Y')}`\nОсталось дней: *{days_left}*"
                else:
                    status = f"🛑 *Истёк* `{exp.strftime('%d.%m.%Y')}`"
            except ValueError:
                status = "⚠️ Ошибка данных"
        else:
            status = "❓ Дата оплаты не задана"

        markup_c = InlineKeyboardMarkup()
        markup_c.add(IBt("💳 Оплатить / Продлить", callback_data="client_pay"))
        markup_c.add(IBt("🛠 Инструкция по подключению", callback_data="client_howto"))
        bot.send_message(message.chat.id, f"📡 {status}", reply_markup=markup_c, parse_mode="Markdown")
    ...
    def cb_extend(call):
        ...
        ip = call.data.split("|")[1]
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(InlineKeyboardButton("+1 месяц",   callback_data=f"extend_confirm|{ip}|30"),
        ...
        markup.add(InlineKeyboardButton("♾️ Бессрочно", callback_data=f"extend_confirm|{ip}|lifetime"))
        markup.add(InlineKeyboardButton("❌ Отмена", callback_data="page|0"))
        bot.edit_message_text(
            f"📅 Сколько продлить IP `{ip}`?\n"
            f"_(прибавляется к текущей дате окончания, дни не сгорают)_",
            call.message.chat.id, call.message.message_id,
            reply_markup=markup, parse_mode="Markdown"
        )
